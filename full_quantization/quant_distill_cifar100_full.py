...
parser.add_argument('--teacher_root', type=str, default='../models_distill/resnet164_distill_cifar100.t7', help='directory of teacher checkpoint.') 
parser.add_argument('--student_root', type=str, default='./models/resnet110_cifar100.t7', help='directory of student checkpoint if fine-tuning.')
...
class PreAct_ResNet_Cifar(nn.Module):
    def __init__(self, block, layers, num_classes=100, noise_coef=None, version = 'v1'):
...
ckpt = torch.load('./models/resnet110_cifar100.t7')
model_temp = ckpt['state']

model_quant = resnet_type_cifar.resnet110(num_classes=100, bit = a_bit, ffun=ffun, bfun=bfun, rate_factor=rate_factor, gd_alpha=gd_alpha, gd_type=gd_type).cuda()
if fine_tuning:
...
#scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[50, 70, 80, 90], gamma=0.1)

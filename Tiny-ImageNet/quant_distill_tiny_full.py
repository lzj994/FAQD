...
parser.add_argument('--student_root', type=str, default='./models/resnet18_pretrain_full_1A.t7', help='directory of student checkpoint if fine-tuning.')
...
def fast_fa_loss(feat, ref_feat, num_ensemble = 15, order = 1):
    torch.manual_seed(1)
    batch_size, ch, h, w = feat.size(0), feat.size(1), feat.size(2), feat.size(3)

    # generating random vector  (HW) x dim
    vec = torch.randn(h*w, num_ensemble).detach().unsqueeze(0).repeat(batch_size,1,1).cuda()
...
if fine_tuning:
    model_quant = resnet_type_tiny.resnet18_quant(bit = a_bit, ffun=ffun, bfun=bfun, rate_factor=rate_factor, gd_alpha=gd_alpha, gd_type=gd_type).cuda()
    ckpt = torch.load(student_root)
    #model_quant.load_state_dict(ckpt['state'])
    model_quant = ckpt['state']
    #model_quant.relu = QuantReLU(a_bit, ffun=ffun, bfun=bfun, rate_factor=rate_factor, gd_alpha=False, gd_type=gd_type)
...
if FA_type == 'FA':
    ...
    FA_loss = lambda feat, ref_feat: fast_fa_loss(feat, ref_feat, num_ensemble = num_ensemble)
